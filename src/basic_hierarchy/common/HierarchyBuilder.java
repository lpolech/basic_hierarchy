...
/**
 * This class exposes methods allowing to build and fix gaps in
 * {@link Hierarchy} objects.
 */
public class HierarchyBuilder {
	...
	/**
	 * Builds a complete hierarchy of nodes, while also patching up holes in the
	 * original hierarchy by inserting empty nodes for missing IDs.
	 * 
	 * @param root           the root node
	 * @param nodes          the original collection of nodes
	 * @param fixBreadthGaps whether the hierarchy fixing algorithm should also fix
	 *                       gaps in breadth, not just depth.
	 * @param useSubtree     whether the centroid calculation should also include
	 *                       child nodes' instances. When set to {@code true}, all
	 *                       objects from subnodes are regarded as also belonging to
	 *                       their supernodes.
	 * @return the complete 'fixed' collection of nodes, filled with artificial
	 *         nodes
	 */
	public List<? extends Node> buildCompleteHierarchy(BasicNode root, List<BasicNode> nodes, boolean fixBreadthGaps,
			boolean useSubtree) {
	...
	/**
	 * Recursively sorts all children of the specified node using
	 * {@link NodeIdComparator}
	 * 
	 * @param root the node to sort
	 */
	...
	/**
	 * Recursively sorts all children of the specified node using the specified
	 * comparator.
	 * 
	 ...
	 * 
	 * @param nodes            collection of all nodes for the centroids are to be
	 *                         recalculated
	 * @param useSubtree       whether the centroid calculation should also include
	...
	/**
	 * Updates all nodes in the specified collection so that their actual
	 * parent-child relations match up with their IDs.
	 * <p>
	 * If a node's ID implies it has a parent, but that parent is not present in the
	 * collection, then that node will not have its parent set.
	 * </p>
	 * <p>
	 * This means that this method DOES NOT GUARANTEE that the hierarchy it creates
	 * will be contiguous. To fix this, follow-up this method with
	 * {@link #fixDepthGaps(BasicNode, List, boolean)} and/or
	 * {@link #fixBreadthGaps(BasicNode, boolean)}
	 * </p>
	 * 
	 * @param nodes            collection of all nodes to build the hierarchy from
	 * @param progressReporter function used to report progress of this operation.
	 *                         Can be null.
	 */
	 ...
	 * <p>
	 * Such gaps appear when the source file did not list these nodes (since they
	 * were empty), but their existence can be inferred from IDs of existing nodes.
	 * </p>
	 * 
	 * @param nodes            the original collection of nodes
	...
	public static List<BasicNode> fixDepthGaps(List<BasicNode> nodes, String rootId, boolean useSubtree,
		...
		for (int i = 0; i < total; ++i) {
			...
			if (!node.getId().equals(rootId) && node.getParent() == null) {
	 ...
	 * 
	 * @param ancestor   the nearest ancestor node that already exists within the
	 *                   hierarchy
	 * @param descendant the descendant node we want to create parents for
	...
	public static List<BasicNode> fixDepthGapsBetween(BasicNode ancestor, BasicNode descendant, boolean useSubtree) {
		...
		for (int j = ancestorHeight; j < descendantHeight - 1; ++j) {
			...
			// Add an empty node
			BasicNode newNode = new BasicNode(newId, newParent, useSubtree);
	...
	/**
	 * Fixes gaps in breadth (missing siblings) by creating empty nodes where
	 * needed.
	 * <p>
	 * Such gaps appear when the source file did not list these nodes (since they
	 * were empty), but their existence can be inferred from IDs of existing nodes.
	 ...
	 * 
	 * @param node       the node to fix breadth gaps in
	 * @param useSubtree whether the centroid calculation should also include child
	 *                   nodes' instances
	 * @return collection of artificial nodes created as a result of this method
	 */
	public static List<BasicNode> fixBreadthGapsInNode(BasicNode node, boolean useSubtree) {
		...
		// Set the list of children of the current node, in case implementation of
		// getChildren()
	...
	/**
	 * Attempts to find the nearest existing node that can act as an ancestor to the
	 * node specified in argument. IF no such node could be found, this method
	 * returns null.
	 * <p>
	 * This method relies on the nodes' IDs being correctly formatted and allowing
	 * us to infer the parent-child relations.
	 * </p>
	 * 
	 * @param idNodeMap map containing all nodes thus far (both real and artificial
	 *                  ones)
	 * @param childId   id of the child node we're trying to find an ancestor for
	...
	private static BasicNode findNearestAncestor(Map<String, BasicNode> idNodeMap, String childId) {
		// Work our way backwards from the given child id, ascending one level after
		// each miss
	...
	/**
	 * @param id the id to get the parent id for
	...
	/**
	 * @param id the id in which it should be checked if contains parent
	 * @return boolean value predicting if it is possible to extract parent id from
	 *         this id
	 */
	...
	/**
	 * @param n the node to compute the node height for
	 * 
	 * @return height of the node (how deep it is within the tree). 1 means root
	 *         node.
	 */
	...
	/**
	 * @param id the id to compute the node height for
	 * 
	 * @return height of the node (how deep it is within the tree). 1 means root
	 *         node.
	 */
	...
	public static boolean areNodesParentAndChild(Node parent, Node child) {
		return areIdsParentAndChild(parent.getId(), child.getId());
	}
	...
	public static boolean areNodesAncestorAndDescendant(Node ancestor, Node descendant) {
		return areIdsAncestorAndDescendant(ancestor.getId(), descendant.getId());
	}

	/**
	 * Checks whether the two IDs represent nodes that are directly related
	 * (parent-child). This method returns false if both IDs point to the same node.
	 * 
	 * @param parentId ID of the node acting as parent
	 * @param childId  ID of the node acting as child
	...
	public static boolean areIdsParentAndChild(String parentId, String childId) {
		return areIdsAncestorAndDescendant(parentId, childId)
				&& childId.indexOf(Constants.HIERARCHY_BRANCH_SEPARATOR, parentId.length() + 1) == -1;
	}

	/**
	 * Checks whether the two IDs represent nodes that are indirectly related
	 * (ancestor-descendant). This method returns false if both IDs point to the
	 * same node.
	 * 
	 * @param ancestorId   ID of the node acting as ancestor
	 * @param descendantId ID of the node acting as descendant
